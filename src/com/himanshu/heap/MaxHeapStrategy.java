...
public class MaxHeapStrategy  implements HeapStrategy {
     ...
     * same then the smaller of two node is inserted in left sub heap. The
     ...
     public void insertNode(HeapNode currentHeapNode, HeapNode newHeapNode) {
        if (currentHeapNode.data.compareTo(newHeapNode.data) > 0) {
            if (currentHeapNode.leftChildHeapNode.getHeapDepth() <= currentHeapNode.rightChildHeapNode.getHeapDepth()) {
                if(currentHeapNode.leftChildHeapNode instanceof HeapNullNode)
                    currentHeapNode.leftChildHeapNode = newHeapNode; // If currentHeapNode rightChildHeapNode is null
                else
                    insertNode(currentHeapNode.leftChildHeapNode, newHeapNode);
            }
            else {
                if (currentHeapNode.rightChildHeapNode instanceof HeapNullNode)
                    currentHeapNode.rightChildHeapNode = newHeapNode;// If currentHeapNode rightChildHeapNode is null
                else
